...
void insert_node_into_doubly_linked_list(DoublyLinkedList **doubly_linked_list,
                                         int node_data)
{
...
DoublyLinkedListNode *reverse(DoublyLinkedListNode *llist)
{
    ...
    if (!llist->next)
        return llist;
    DoublyLinkedListNode *new_head = reverse(llist->next);
    llist->next->prev = llist->next->next;
...
int main()
{
    ...
    for (int t_itr = 0; t_itr < t; t_itr++)
    {
        ...
        if (llist_count_endptr == llist_count_str ||
            *llist_count_endptr != '\0')
        {
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < llist_count; i++)
        {
            ...
            if (llist_item_endptr == llist_item_str ||
                *llist_item_endptr != '\0')
            {
                exit(EXIT_FAILURE);
            }
