...
hashNode **createTab()
{
    hashNode **tab = calloc(HASHSIZE, sizeof(hashNode *));
    return tab;
}

hashNode *lookup(hashNode **tab, char *s)
{
    for (hashNode *np = tab[hash(s)]; np; np = np->next)
    {
        if (strcmp(np->s, s) == 0)
        {
            return np;
...
hashNode *install(hashNode **tab, char *s)
{
    hashNode *np = lookup(tab, s);
    if (!np)
    {
        np = malloc(sizeof(hashNode));
        np->count = 1;
        np->s = strdup(s);
        np->next = tab[hash(s)];
        tab[hash(s)] = np;
...
int getStringCount(hashNode **tab, char *s)
{
    hashNode *np = lookup(tab, s);
    if (np)
    {
...
{
    hashNode **hashTab = createTab();
    for (int i = 0; i < strings_count; ++i)
    {
        install(hashTab, strings[i]);
    }
    int *result = malloc(sizeof(int) * queries_count);
    *result_count = queries_count;
    for (int i = 0; i < queries_count; ++i)
    {
        result[i] = getStringCount(hashTab, queries[i]);
    }
    return result;
}
...
int main()
{
    ...
    int res_count;
    int *res = matchingStrings(strings_count, strings, queries_count, queries,
                               &res_count);
